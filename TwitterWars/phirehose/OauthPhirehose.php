...
abstract class OauthPhirehose extends Phirehose
{
    ...
    * If not set then the global define CONSUMER_KEY is used instead.
    */
    public $consumerKey=null;

    /**
    * The Twitter consumer secret. Get it from the application's page on Twitter.
    * If not set then the global define CONSUMER_SECRET is used instead.
    */
    ...
    */
		...
		array $params)
	{
		...
		$oauth['oauth_consumer_key'] = $this->consumerKey?$this->consumerKey:CONSUMER_KEY;
...
//                
//                $ts = $oauth['oauth_timestamp'];
//                $date = new DateTime("@$ts");
//                echo "time : " . $date->format('U = Y-m-d H:i:s') . "\n";
		...
		$params = null)
	{
		...
		$key = $this->encode_rfc3986($this->consumerSecret?$this->consumerSecret:CONSUMER_SECRET) . '&' . $this->encode_rfc3986($this->password);
